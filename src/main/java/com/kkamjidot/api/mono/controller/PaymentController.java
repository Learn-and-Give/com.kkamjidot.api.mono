...
import com.kkamjidot.api.mono.dto.response.enumerate.PointResponse;
...
public class PaymentController {
    ...
    public ResponseEntity<PointResponse> readQuiz(@RequestHeader String jwt) {
        ...
        Integer countOfAllQuizzesSolvedByUser = solveRepository.countOfAllQuizzesSolvedByUser(user.getId());

        return ResponseEntity.ok(PointResponse.builder().userId(user.getId()).point(countOfAllQuizzesSolvedByUser * 100).build());
    }
}