...
public class QuizController {
    ...
    @Operation(summary = "개발 중)퀴즈 전체 내용 조회 API", description = "퀴즈의 정답을 포함한 모든 정보를 조회한다. 열람 가능 주차가 아니면 403 에러를 반환한다. 단, 작성자가 본인일 경우 열람 가능하다.")
    ...
    public ResponseEntity<QuizIdResponse> createQuiz(@Parameter(description = "로그인한 회원 코드", example = "1234") @RequestHeader String code,
        ...
        LOGGER.info("API: v1/challenges/{challengeId}/quizzes [path: challengeId = {}] [body: createQuizRequest = {}, num of quizFiles = {}]", challengeId, createQuizRequest, quizFiles.size());
